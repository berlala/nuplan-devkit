...
def sampled_tracked_objects_to_tensor_list(past_tracked_objects):
    ...
    for i in range(len(past_tracked_objects)):
        tensorized, track_token_ids, agent_types = _extract_agent_tensor(
            past_tracked_objects[i], track_token_ids, object_types
        )
...
def convert_absolute_quantities_to_relative(agent_state, ego_state, agent_type="ego"):
    ...
    if agent_type == "ego":
        ...
        transformed_poses = global_state_se2_tensor_to_local(
            agent_global_poses, ego_pose, precision=torch.float64
        )
        ...
        agent_global_velocities = agent_state[
            :, [AgentInternalIndex.vx(), AgentInternalIndex.vy()]
        ]
        transformed_poses = global_state_se2_tensor_to_local(
            agent_global_poses, ego_pose, precision=torch.float64
        )
        transformed_velocities = global_velocity_to_local(
...
def agent_past_process(
        ...
        for agent_state in padded_agent_states:
            local_coords_agent_states.append(
                convert_absolute_quantities_to_relative(
                    agent_state, anchor_ego_state, "agent"
                )
            )

        # Calculate yaw rate
        yaw_rate_horizon = compute_yaw_rate_from_state_tensors(
            padded_agent_states, time_stamps
        )
    ...
    """
    agents = np.zeros(
        shape=(num_agents, agents_tensor.shape[0], agents_tensor.shape[-1] + 3),
        dtype=np.float32,
    )
    ...
    for i, j in enumerate(indices):
        agents[i, :, : agents_tensor.shape[-1]] = agents_tensor[
            :, j, : agents_tensor.shape[-1]
        ].numpy()
...
def agent_future_process(
    anchor_ego_state, future_tracked_objects, num_agents, agent_index
):
...
def pad_agent_states_with_zeros(agent_trajectories):
    ...
    pad_agent_trajectories = torch.zeros(
...
def convert_feature_layer_to_fixed_size(
    ...
    tl_data_tensor = (
        torch.zeros(
            (max_elements, max_points, traffic_light_encoding_dim), dtype=torch.float32
        )
...
def polyline_process(polylines, avails, traffic_light=None):
    ...
    new_polylines = np.zeros(
        shape=(polylines.shape[0], polylines.shape[1], dim), dtype=np.float32
    )

    for i in range(polylines.shape[0]):
        if avails[i][0]:
            polyline = polylines[i]
            polyline_heading = wrap_to_pi(
                np.arctan2(
                    polyline[1:, 1] - polyline[:-1, 1],
                    polyline[1:, 0] - polyline[:-1, 0],
                )
            )
            ...
            if traffic_light is None:
                ...
                new_polylines[i] = np.concatenate(
                    [polyline, polyline_heading, traffic_light[i]], axis=-1
                )
...
def encoding_vectornet_features(
    agents_past, agents_future, ego_past, ego_future, vector_map
):
    ...
    trajectory_subgraph = np.vstack((trajectory_subgraph, ego_subgraph))
    ...
    ego_ground_truth = agent_ground_truth_traj_to_increments(
        ego_past[-1, 0:2], ego_future[:, 0:2]
    )

    ground_truth = np.vstack((ground_truth, ego_ground_truth))
    ...
    for i in range(len(agents_past)):
        ...
        trajectory_subgraph = np.vstack((trajectory_subgraph, agent_subgraph))
        ...
        ground_truth = np.vstack((ground_truth, agent_ground_truth))
    ...
    for lane in vector_map["lanes"]:
        ...
        map_subgraph = np.vstack((map_subgraph, lane_subgraph))
...
def decoding_features(observations):
    ...
    crosswalk_index_end = crosswalk_index_start + config.CROSSWALKS_NUM * (
        config.CROSSWALKS_POINTS_NUM - 1
    )
    ...
    for i in range(config.NUM_AGENTS + 1):
        agent_data = observations[
            i * config.NUM_PAST_POSES : (i + 1) * config.NUM_PAST_POSES, :4
        ]
    ...
    for i in range(config.LANE_NUM):
        lane_data = observations[
            lane_index_start
            + i * (config.LANE_POINTS_NUM - 1) : lane_index_start
            + (i + 1) * (config.LANE_POINTS_NUM - 1),
            :4,
        ]
    ...
    for i in range(config.CROSSWALKS_NUM):
        crosswalk_data = observations[
            crosswalk_index_start
            + i * (config.CROSSWALKS_POINTS_NUM - 1) : crosswalk_index_start
            + (i + 1) * (config.CROSSWALKS_POINTS_NUM - 1),
            :4,
        ]
    ...
    for i in range(config.ROUTE_LANES_NUM):
        route_lane_data = observations[
            route_lane_index_start
            + i * (config.ROUTE_LANES_POINTS_NUM - 1) : route_lane_index_start
            + (i + 1) * (config.ROUTE_LANES_POINTS_NUM - 1),
            :4,
        ]
...
def increment_to_trajectories(predictions, agent_current_pose_list):
    ...
    for i in range(1, config.NUM_FUTURE_POSES):
        predicted_path_list[:, i, :] = (
            predicted_path_list[:, i - 1, :] + increments[:, i, :]
        )
...
def create_map_raster(lanes, crosswalks, route_lanes):
    for lane in lanes:
        if lane[0][0] != 0:
            plt.plot(
                lane[:, 0], lane[:, 1], color=COLOR_DICT["centerline"], linewidth=1
            )  # plot centerline
