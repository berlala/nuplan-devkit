...
def get_sequential_edge_index(num_nodes, start=0):
    ...
    edge_indices = np.array(
        [[i, i + 1] for i in range(start, start + num_nodes - 1)], dtype=np.int64
    ).T
        ...
        for data_path in tqdm(data_path_list):
            ...
            for id, mask in trajectory_ID_to_indices.items():
                ...
                edge_index = get_sequential_edge_index(
                    trajectory_features.shape[0], start=mask[0]
                )
            ...
            for id, mask in lane_ID_to_indices.items():
                lane_features = features[
                    accumulate_edge_index
                    + mask[0] : accumulate_edge_index
                    + mask[1]
                    + 1
                ]
                ...
                # )
                edge_index = get_sequential_edge_index(
                    lane_features.shape[0], start=mask[0] + accumulate_edge_index
                )
            ...
            graph_data = GraphData(
                ...
                time_step_len=torch.tensor(
                    [trajectory_features.shape[0]], dtype=torch.long
                ),
            )
